# Copied and adapted from Andrej Karpathy's nanoGPT repo
...
def get_batch(split):
    ...
    if split == 'train':
        data = np.memmap(os.path.join(args.data_dir, 'train.bin'), dtype=np.uint16, mode='r')
    else:
...
scaler = torch.cuda.amp.GradScaler(device_type, enabled=(dtype == 'float16'))
